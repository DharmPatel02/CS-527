...
const CustomerRepresentative = () => {
  ...
  const fetchPasswordRequests = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch("http://localhost:8080/auth/null-passwords", {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleSetPassword = async (userId) => {
    ...
    if (!newPassword.trim()) {
      setError("Please enter a new password for user ID " + userId);
      return;
    }
    ...
    try {
      ...
      // Refresh the password requests list
