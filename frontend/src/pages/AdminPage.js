...
//     let url = "http://localhost:8080/auth/auction-items";
...
const AdminPage = () => {
  ...
  const handleGetBids = async () => {
    ...
    const url = `http://localhost:8080/auth/bids/${auctionId}/bids`;
  ...
  const handleRemoveBid = async (bidId) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/auth/bids/remove_bid/${bidId}/${auctionId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
