...
public class AuctionEventScheduler {
    ...
    @Autowired
    public AuctionEventScheduler(Scheduler scheduler, AuctionItemsRepository itemsRepo) {
        this.scheduler = scheduler;
        this.itemsRepo = itemsRepo;
    }

    /**
     * Delay scheduling until after Render detects the open port.
     * Executes once 10 seconds after application startup.
     */
    @PostConstruct
    @Scheduled(initialDelay = 10000, fixedDelay = Long.MAX_VALUE)
    public void deferScheduleAllAuctions() {
        try {
            scheduleAllAuctions();
        } catch (SchedulerException e) {
            log.error("❌ Failed to schedule auctions: {}", e.getMessage(), e);
        }
    }

    /**
     * Schedule start and end jobs for all future auctions.
     */
    public void scheduleAllAuctions() throws SchedulerException {
        var jobKeys = scheduler.getJobKeys(GroupMatcher.jobGroupEquals("auction-end"));
        if (!jobKeys.isEmpty()) {
        ...
        for (AuctionItems a : all) {
            Date closeAt = a.getClosingTime();
            boolean future = closeAt.after(now);
            log.info("→ auction {} closes at {} (future? {})", a.getauction_id(), closeAt, future);
            ...
            if (future) {
                ...
                if (!scheduler.checkExists(endJobKey) && !scheduler.checkExists(endTrigKey)) {
                    ...
                    log.info("⚠️ Job or trigger already exists for auction {}, skipping...", a.getauction_id());
                }
            }
        }
    }
    ...
    /**
     * Used when creating a new auction to schedule its end.
     */
    public void scheduleEndForAuction(AuctionItems a) throws SchedulerException {
        Date closeAt = a.getClosingTime();
        if (closeAt.after(new Date())) {
            ...
            if (!scheduler.checkExists(endJobKey) && !scheduler.checkExists(endTrigKey)) {
                ...
                log.info("⚠️ Job or trigger already exists for auction {}, skipping schedule...", a.getauction_id());
            }
        }
    }
}
